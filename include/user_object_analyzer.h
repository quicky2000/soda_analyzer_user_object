...
namespace osm_diff_analyzer_user_object
{
  ...
  private:
    ...
    inline void compare(const osm_api_data_types::osm_core_element::t_osm_version p_previous_version,
    ...
    inline void compare(const osm_api_data_types::osm_core_element & p_core_element1,
					       ...
					       const osm_api_data_types::osm_change::t_osm_change_type & p_change)
    {
      ...
      if(l_casted_object->get_user() == m_user_name && !m_db.contains(l_casted_object))
      ...
      else if(m_db.contains(l_casted_object) && l_casted_object->get_user() != m_user_name)
        {
          ...
          if(l_modified)
            {
              compare(l_previous_version,*l_casted_object);
            }
  ...
  //------------------------------------------------------------------------------
  void user_object_analyzer::compare(const osm_api_data_types::osm_core_element::t_osm_version p_previous_version,
				     const osm_api_data_types::osm_node & p_node2)
  {
        ...
        if(l_redacted)
          {
            ...
            m_report << "<B>Due to redaction process comparison is done against <A HREF=\""<< l_redacted_link << "\">" << p_node2.get_core_type_str() << " " << p_node2.get_id() << " v" << l_redacted_version << "</A></B><BR>" << std::endl ;
            m_api.cache(p_node2);
          }

        m_report << "<ul>" << std::setprecision(9);
        if(l_node1->get_lat() != p_node2.get_lat() && l_node1->get_lon() != p_node2.get_lon())
          {
            m_report << "<li>Coordinates has changed from (" << l_node1->get_lat() << "," << l_node1->get_lon() << ") to (" << p_node2.get_lat() << "," << p_node2.get_lon() << ")</li>" << std::endl ;
          }
        else if(l_node1->get_lat() != p_node2.get_lat())
          {
            m_report << "<li>Latitude has changed from " << l_node1->get_lat() << " to " << p_node2.get_lat() << "</li>" << std::endl ;
          }
        else if(l_node1->get_lon() != p_node2.get_lon())
          {
            m_report << "<li>Longitude has changed from " << l_node1->get_lon() << " to " << p_node2.get_lon() << "</li>" << std::endl ;
          }
        compare(*((const osm_api_data_types::osm_core_element *)l_node1),*((const osm_api_data_types::osm_core_element *)&p_node2));
        m_report << "</ul>" ;
        delete l_node1;
      }
    else
      {
        m_report << "<B>Unable to perform comparison because object had been redacted</B><BR>" << std::endl ;
      }
  }
        ...
        if(l_redacted)
          {
            ...
            m_report << "<B>Due to redaction process comparison is done against <A HREF=\""<< l_redacted_link << "\">" << p_way2.get_core_type_str() << " " << p_way2.get_id() << " v" << l_redacted_version << "</A></B><BR>" << std::endl ;
            m_api.cache(p_way2);
          }

        m_report << "<ul>" ;
        compare(*((const osm_api_data_types::osm_core_element *)l_way1),*((const osm_api_data_types::osm_core_element *)&p_way2));
            ...
            if(l_nodes_way1.find(*l_iter)==l_nodes_way1.end())
              {
                ...
                m_report << "<li><A HREF=\"" << l_node_url << "\">Node " << *l_iter << "</A> has been added to <A HREF=\"" << l_way_url << "\">Way " << p_way2.get_id() << "</A></li>" << std::endl ;
              }
          }
        for(std::vector<osm_api_data_types::osm_object::t_osm_id>::const_iterator l_iter = l_way1->get_node_refs().begin();
            l_iter != l_way1->get_node_refs().end();
            ...
            if(l_nodes_way2.find(*l_iter)==l_nodes_way2.end())
              {
                std::string l_node_url;
                m_api.get_object_browse_url(l_node_url,"node",*l_iter);
                m_report << "<li><A HREF=\"" << l_node_url << "\">Node " << *l_iter << "</A> has been removed from <A HREF=\"" << l_way_url << "\">Way " << p_way2.get_id() << "</A></li>" << std::endl ;
              }
          }
        m_report << "</ul>" ;
        delete l_way1;
        ...
        if(l_redacted)
          {
            ...
            m_report << "<B>Due to redaction process comparison is done against <A HREF=\""<< l_redacted_link << "\">" << p_relation2.get_core_type_str() << " " << p_relation2.get_id() << " v" << l_redacted_version << "</A></B><BR>" << std::endl ;
            m_api.cache(p_relation2);
          }

        m_report << "<ul>" ;
        compare(*((const osm_api_data_types::osm_core_element *)l_relation1),*((const osm_api_data_types::osm_core_element *)&p_relation2));
            ...
            if(l_member_iter==l_members_relation1.end())
              {
                ...
                m_report << "<li><A HREF=\"" << l_object_url << "\">" << l_type_str << " " << l_member_id << "</A> has been added with role \"" << (*l_iter)->get_role() << "\"</li>" << std::endl ;
              }
            else if(l_member_iter->second->get_role() != (*l_iter)->get_role())
              {
                ...
                std::string l_object_url;
                m_api.get_object_browse_url(l_object_url,l_type_str,l_member_id);
                m_report << "<li>Role of <A HREF=\"" << l_object_url << "\">" << l_type_str << " " << l_member_id << "</A> has changed from \"" << l_member_iter->second->get_role() << "\" to \"" << (*l_iter)->get_role() << "\"</li>" << std::endl ;
              }
          }
        for(std::vector<osm_api_data_types::osm_relation_member*>::const_iterator l_iter = l_relation1->get_members().begin();
            l_iter != l_relation1->get_members().end();
            ...
            if(l_member_iter==l_members_relation2.end())
              {
                ...
                std::string l_object_url;
                m_api.get_object_browse_url(l_object_url,l_type_str,l_member_id);
                m_report << "<li><A HREF=\"" << l_object_url << "\">" << l_type_str << " " << l_member_id << "</A> has been removed</li>" << std::endl ;
              }
          }
        m_report << "</ul>" ;
        delete l_relation1;
  ...
  //------------------------------------------------------------------------------
  void user_object_analyzer::compare(const osm_api_data_types::osm_core_element & p_core_element1,
				     const osm_api_data_types::osm_core_element & p_core_element2)
  {
	...
	if(p_core_element2.contains_tag(l_iter->first))
	  {
	    ...
	    if(l_new_value != l_iter->second)
	      {
		m_report << "<li>Tag \"" << l_iter->first << "\" value has changed from \"" << l_iter->second << "\" to \"" << l_new_value << "\"</li>" << std::endl ;
	      }
	  }
	else
	  {
	    m_report << "<li>Tag \"" << l_iter->first << "\" with value \"" << l_iter->second << "\" has been removed</li>" << std::endl ;
	  }
      }
	...
	if(!p_core_element1.contains_tag(l_iter->first))
	  {
	    m_report << "<li>Tag \"" << l_iter->first << "\" with value \"" << l_iter->second << "\" has been added</li>" << std::endl ;
	  }
      }
